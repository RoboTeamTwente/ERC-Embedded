...
static result_t handle_arm_control_signals(void* buffer) {
  if (buffer == NULL) { return RESULT_ERR_INVALID_ARG; }
...
static result_t handle_sensor_pump_command(void* buffer) {
  ...
  LOGI(TAG, "Pump cmd: enabled=%d dir=%d speed=%lu%%",
       cmd->enabled, cmd->direction,
       (unsigned long)cmd->speed_percent);
...
static void sensor_udp_rx_callback(void *payload, size_t length,
  ...
  if (payload == NULL || length == 0U || addr == NULL) { return; }
...
static result_t send_sensor_diagnostics_udp(uint8_t dest_ip[4], uint8_t port,
                                            ...
                                            uint8_t prio) {
  if (diagnostics == NULL) { return RESULT_ERR_INVALID_ARG; }
  ...
  if (r != RESULT_OK) { LOGE(TAG, "Encode diag failed: %s", result_to_short_str(r)); return r; }
  r = ETH_udp_send_binary(dest_ip, port, enc, enc_sz, prio);
  if (r != RESULT_OK) { LOGE(TAG, "Send diag failed: %s", result_to_short_str(r)); }
  free(enc);
  return r;
}
...
static result_t send_load_cell_info_udp(uint8_t dest_ip[4], uint8_t port,
                                        ...
                                        uint8_t prio) {
  if (info == NULL) { return RESULT_ERR_INVALID_ARG; }
  PBEnvelope envelope = PBEnvelope_init_zero;
...
static result_t send_flow_sensor_info_udp(uint8_t dest_ip[4], uint8_t port,
  ...
  r = ETH_udp_send_binary(dest_ip, port, enc, enc_sz, prio);
  if (r != RESULT_OK) { LOGE(TAG, "Send flow sensor failed: %s", result_to_short_str(r)); }
  free(enc);
  return r;
}
...
static result_t send_pump_info_udp(uint8_t dest_ip[4], uint8_t port,
  ...
  r = ETH_udp_send_binary(dest_ip, port, enc, enc_sz, prio);
  if (r != RESULT_OK) { LOGE(TAG, "Send pump info failed: %s", result_to_short_str(r)); }
  free(enc);
  return r;
}
...
PACKET_HANDLER_CONFIG_STATIC(sensor_pump_cmd_handler,
                             PBEnvelope_pump_info_tag,
                             pump_info,
...
extern TIM_HandleTypeDef htim3;   /* PWM timer for pump — adjust to your CubeMX config */
...
void MainTask(void *argument) {
  ...
  for (size_t i = 0; i < 2; i++) { load_cell_sensor_init(&load_cell_data[i]); }

  pressure_sensor_data_t pressure_data[2];
  LOGI(TAG, "Initializing Pressure Sensors...");
  for (size_t i = 0; i < 2; i++) { pressure_sensor_init(&pressure_data[i]); }
  ...
  if (flow_init_result != RESULT_OK) {
    LOGE(TAG, "Flow sensor init failed: %s", result_to_short_str(flow_init_result));
  } else {
  ...
  static uint8_t txStorage0[ETHERNET_RQ_LENGTH * sizeof(udp_send_frame_t)];
  QueueHandle_t tx0 = xQueueCreateStatic(ETHERNET_RQ_LENGTH,
                                        sizeof(udp_send_frame_t),
                                        txStorage0, &txStruct0);

  static StaticQueue_t txStruct1;
  static uint8_t txStorage1[ETHERNET_RQ_LENGTH * sizeof(udp_send_frame_t)];
  QueueHandle_t tx1 = xQueueCreateStatic(ETHERNET_RQ_LENGTH,
                                       sizeof(udp_send_frame_t),
  ...
  while (1) {
    ...
    for (size_t i = 0; i < 2; i++) {
      ...
      if (lc_result == RESULT_ERR_UNIMPLEMENTED || lc_result == RESULT_ERR_COMMS) {
        ...
        LOGE(TAG, "Load cell %lu - Poll error: %d", (unsigned long)i, lc_result);
      ...
      if (send_r != RESULT_OK) {
        LOGE(TAG, "Load cell %lu UDP send failed: %s",
             (unsigned long)i, result_to_short_str(send_r));
      }
      ...
      if (pr_result == RESULT_ERR_UNIMPLEMENTED || pr_result == RESULT_ERR_COMMS) {
        ...
        LOGE(TAG, "Pressure %lu - Poll error: %d", (unsigned long)i, pr_result);
     ...
     */
    {
      ...
      result_t flow_send_r = send_flow_sensor_info_udp(dest_ip, SENSOR_UDP_DEST_PORT,
                                                       &flow_info, 1);
     ...
     */
    {
      ...
      if (!g_pump_data.is_initialised) {
        ...
        pump_info.state  = g_pump_data.enabled
                               ? SensorState_SENSOR_OPERATING
                               : SensorState_SENSOR_IDLE;
        pump_info.status = SensorStatus_STATUS_OK;
      }

      result_t pump_send_r = send_pump_info_udp(dest_ip, SENSOR_UDP_DEST_PORT,
                                                &pump_info, 1);
