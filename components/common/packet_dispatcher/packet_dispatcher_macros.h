...
#define PACKET_HANDLER_CONFIG_STATIC_QUEUE(name, packet_tag, payload_member,   \
                                           handler_fn, queue_length_)          \
    static uint8_t                                                             \
...
#define PACKET_HANDLER_CONFIG_STATIC_PRIO(name, packet_tag, payload_member,    \
                                          handler_fn, priority_)               \
    static uint8_t                                                             \
        name##_queue_buffer[PACKET_HANDLER_DEFAULT_QUEUE_LENGTH *              \
                            sizeof(((PBEnvelope*)0)->payload.payload_member)]; \
    static packet_handler_config_t name = {                                    \
...
#define PACKET_HANDLER_CONFIG_STATIC_PRIO_QUEUE(                               \
    name, packet_tag, payload_member, handler_fn, queue_length_, priority_)    \
    static uint8_t                                                             \
        name##_queue_buffer[queue_length_ *                                    \
                            sizeof(((PBEnvelope*)0)->payload.payload_member)]; \
    static packet_handler_config_t name = {                                    \
