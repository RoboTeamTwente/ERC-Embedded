...
/**
 * @brief Initialize the logging system for the sensor board.
 * @param arg a pointer to a UART_HandleTypeDef
 */
void LOG_init(void *arg) {
  huart_handler = *(UART_HandleTypeDef *)arg;
  initialized = 1;
  LOG(LOG_INFO, "LOGGING", "Sensor Board Logging Initialized");
}

/**
 * @brief Log a message with a specified log level and tag.
 * @param level The log level (LOG_INFO, LOG_WARNING, LOG_ERROR)
 * @param TAG The tag representing the source of the log
 * @param log_message The message format string (printf-style)
