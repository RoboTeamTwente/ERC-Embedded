...
static cubemars_ak_can_frame motor_frame = {0};
static QueueHandle_t cubemars_ak_queue;
...
void HAL_FDCAN_RxFifo0Callback(FDCAN_HandleTypeDef* hfdcan,
    ...
    if (HAL_FDCAN_GetRxMessage(hfdcan, FDCAN_RX_FIFO0, &rx_header,
                               motor_frame.data) != HAL_OK) {
        LOGE("CAN", "RX read failed, err=0x%08lx", HAL_FDCAN_GetError(hfdcan));
        return;
    }
    xQueueSend(cubemars_ak_queue, &motor_frame, portMAX_DELAY);
}
...
int main() {
    ...
    cubemars_ak_feedback_task_init(cubemars_ak_queue, 30,
                                   tskIDLE_PRIORITY + 2U);
