...
static void CAN_SendTestFrame(FDCAN_HandleTypeDef* handler) {
    ...
    if (HAL_FDCAN_AddMessageToTxFifoQ(handler, &can_test_header, data) !=
        HAL_OK) {
        LOGE("CAN", "TX failed, err=0x%08lx", HAL_FDCAN_GetError(handler));
    } else {
