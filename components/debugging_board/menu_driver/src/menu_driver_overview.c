...
result_t get_menu_page_overview(
    uint8_t id, uint8_t parent_id, char name[MAX_PAGE_NAME_LEN],
    uint8_t num_entries,
    char entries_titles[MENU_OVERVIEW_MAX_ENTRIES]
                       [MENU_OVERVIEW_MAX_ENTRY_TITLE_LEN],
    uint8_t entry_lookups[MENU_OVERVIEW_MAX_ENTRIES], kv_pool* kv_pool,
    menu_page_state* state_ptr, menu_page_t* out_page) {
...
void menu_overview_get_last_states(page_overview_state* state,
    ...
    for (int i = 0; i < state->num_entries; i++) {
        res = kv_pool_get(state->kv_pool, state->entry_lookups[i], &buffer,
                          &size);
...
void menu_page_overview_render_entry(uint8_t offset_x, uint8_t offset_y,
    ...
    if (code != 0x00) {
        ...
        ILI9341_Draw_Bitmap(offset_x + 2, offset_y + 2,
    ...
    snprintf(display_title, sizeof(display_title), title_format,
             formatted_title, code);
    LOGI(TAG, "Rendering overview entry: %s", display_title);
    ILI9341_WriteString(offset_x + MENU_OVERVIEW_ICON_WIDTH + 6, offset_y + 2,
                        display_title, ILI9341_Font_11x18,
                        MENU_DRIVER_FOREGROUND_COLOR,
                        MENU_DRIVER_BACKGROUND_COLOR);
...
void menu_page_render_overview(struct menu_manager_t* manager_str) {
    ...
    for (uint8_t i = 0; i < state->num_entries; i++) {
        ...
        menu_page_overview_render_entry(
            offset_x, offset_y, state->entry_titles[i], state->last_states[i],
