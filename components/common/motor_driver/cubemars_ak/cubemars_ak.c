...
#define CUBEMARS_AK_PI 3.14159265f
...
result_t cubemars_ak_parse_can_feedback(const FDCAN_RxHeaderTypeDef *rx_header,
  ...
  out->motor_temperature = (int8_t)data[6] / CUBEMARS_AK_CAN_TEMPERATURE_SCALE;
...
result_t cubemars_ak_set_speed(FDCAN_HandleTypeDef *can_handler,
  ...
  erpm = (int32_t)__REV(
      (uint32_t)erpm); // You can hate me for this, but its funky and I like it
...
result_t cubemars_ak_set_brake_current(FDCAN_HandleTypeDef *can_handler,
  ...
  int32_t current_raw = (int32_t)(current_a * CUBEMARS_AK_BRAKE_CURRENT_SCALE);
...
static result_t
cubemars_ak_uart_send_motor_id_request(UART_HandleTypeDef *uart_handler) {
...
static result_t
cubemars_ak_uart_receive_motor_id_response(UART_HandleTypeDef *uart_handler,
                                           uint8_t *rx) {
...
uint16_t cubemars_ak_scurve_generate(int32_t current_erpm, int32_t target_erpm,
                                     float peak_accel_erpm_s,
                                     float pole_rate_hz, int32_t *out_speeds,
                                     uint16_t max_count) {
  if (out_speeds == NULL || max_count == 0u || peak_accel_erpm_s <= 0.0f ||
      pole_rate_hz <= 0.0f) {
    return 0u;
  }

  float delta = (float)(target_erpm - current_erpm);
  if (delta == 0.0f) {
    return 0u;
  }

  float T = fabsf(delta) * CUBEMARS_AK_PI / (2.0f * fabsf(peak_accel_erpm_s));
  uint16_t n = (uint16_t)(T * pole_rate_hz + 0.5f);
  if (n < 1u)
    n = 1u;
  if (n > max_count)
    n = max_count;
  ...
  for (uint16_t i = 0u; i < n; i++) {
    float t = (float)(i + 1u) / (float)n;
    out_speeds[i] = (int32_t)((float)current_erpm +
                              delta * 0.5f * (1.0f - cosf(CUBEMARS_AK_PI * t)));
  }
  out_speeds[n - 1u] = target_erpm;
