...
void ILI9341_Draw_Colour_Array(const uint16_t* Colour, uint32_t PixelCount) {
    ...
    while (pixel_index < PixelCount) {
        ...
        for (uint32_t i = 0; i < pixels_to_send; i++) {
            ...
            tx_buf[(i * 2U) + 0U] =
                (uint8_t)(c >> 8);  // MSB first (big-endian)
...
static inline void print_bitmap(uint8_t** bitmap, size_t width, size_t height) {
    ...
    for (uint8_t y = 0; y < height; y++) {
        for (uint8_t x = 0; x < width; x++) {
            bool set =
                ((*bitmap)[y * stride + (x >> 3)] & (0x80u >> (x & 7u))) != 0u;
            printf("%c", set ? '#' : '.');
        }
        printf("\n");
    }
...
static inline result_t ILI9341_Get_Rounded_Corner_Bitmap(
    uint8_t radius, uint8_t thickness, uint8_t* bitmap, uint32_t bitmap_bytes) {
...
void ILI9341_Draw_Rectangle_Custom_Corner(uint16_t X, uint16_t Y,
    ...
    //
    //
    ILI9341_Draw_Rectangle(X + corner_size, Y, Width - corner_size * 2,
                           thickness, Color);
    // Bottom Line
