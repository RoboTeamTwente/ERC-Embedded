...
//! NOTE: this is assuming that all steppers are the same stepper and thus have
//! the same specs
#define STEPPER_PWM_TIMER_TICK_HZ                                              \
  1000000U // Amt of Herz we calculate with after setting the prescaler (should
           // be 1Mhz)
...
#define DEGREES_PER_STEP                                                       \
  (360 / STEPS_PER_REV) // At 200, 1 step turns the motor 1.8 degrees
...
typedef struct {
  ...
  pin_t dir_pin; // Directoion pin: sets the direction (1 clockwise or 0
                 // counterclockwise)
  pin_t ena_pin; // Enable pin: should be on when using stepper
} stepper_t;

result_t init_stepper(stepper_t *stepper, uint8_t duty_cycle,
                      TIM_HandleTypeDef *htim, pin_t dir_pin, pin_t ena_pin);
