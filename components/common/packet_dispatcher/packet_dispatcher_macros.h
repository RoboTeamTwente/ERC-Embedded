...
#define PACKET_HANDLER_CONFIG_STATIC(name, packet_tag, payload_member_size,    \
                                     handler_fn)                               \
    static uint8_t                                                             \
...
#define PACKET_HANDLER_CONFIG_STATIC_EX(name, packet_tag, payload_member_size, \
                                        handler_fn, priority_, stack_depth_,   \
                                        queue_length_)                         \
    static uint8_t                                                             \
        name##_queue_buffer[(queue_length_) * payload_member_size)];           \
    static packet_handler_config_t name = {                                    \
        .handler = (handler_fn),                                               \
        .task_name = #name,                                                    \
        .packet_type = (packet_tag),                                           \
