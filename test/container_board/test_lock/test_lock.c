#include "lock.h" // The library we are testing
#include "logging.h"
#include <unity.h>

lock_t lock = {1,2,3};
lock_t* lock_ptr = &lock;
...
void setUp(void) {
    // set stuff up here
    lock_init(lock_ptr, angle_close, angle_open, min_pw, max_pw);
}
...
void test_function_open_lock(void) {
    lock_open(lock_ptr);
    TEST_ASSERT_EQUAL_INT(lock.servo_pos, 180);
    TEST_ASSERT_EQUAL_INT(lock.servo_pulse_width, 2000);
}
