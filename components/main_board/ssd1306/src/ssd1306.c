...
void ssd1306_WriteStringUnderlined(char *str, SSD1306_Font_t Font,
                                   SSD1306_COLOR color, uint8_t stroke_width) {
  uint8_t rect_start_x = SSD1306.CurrentX;
  ssd1306_WriteString(str, Font, color);
  uint8_t rect_start_y = SSD1306.CurrentY + 1 + Font.height;
  ssd1306_FillRectangle(rect_start_x, rect_start_y, SSD1306.CurrentX,
                        rect_start_y + stroke_width - 1, color);
}

void ssd1306_WriteStringUnderlinedLine(char *str, SSD1306_Font_t Font,
                                       SSD1306_COLOR color,
                                       uint8_t stroke_width,
                                       uint8_t end_of_line_offset) {
  uint8_t rect_start_x = SSD1306.CurrentX;
  ssd1306_WriteString(str, Font, color);
  uint8_t rect_start_y = SSD1306.CurrentY + 1 + Font.height;
  ssd1306_FillRectangle(rect_start_x, rect_start_y,
                        SSD1306_WIDTH - end_of_line_offset,
                        rect_start_y + stroke_width - 1, color);
}
/* Position the cursor */
